...
void RaceScene::update_buffer(Framebuffer& buffer) {
    ...
    for (size_t y = height / 2; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            ...
            float perspective = (float)y / height / 2.0f;
            ...
            size_t left_grass = (middle_point - road_width - clip_width) * width;
            size_t left_clip = (middle_point - road_width) * width;
            ...
            } else if (x >= left_grass && x < left_clip) {
                draw_pixel(buffer, x, y, RED);
            } else if (x >= left_clip && x < right_clip) {
                draw_pixel(buffer, x, y, GREY);
            } else if (x >= right_clip && x < right_grass) {
                draw_pixel(buffer, x, y, RED);
            } else if (x >= right_grass && x < width) {
                draw_pixel(buffer, x, y, GREEN);
            }
        }
    }
